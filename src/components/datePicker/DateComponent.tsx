...
const DateComponent = ({
	...
	const isDisable = (viewDate: string, day: number | null) => {
		...
		const start = new Date(disableStart);
		const end = new Date(disableEnd);
		const view = new Date(viewDate);
		view.setDate(day);
  
		return start > view || end < view;
	};
			...
			{weeks.map((weekDate, index) => (
				...
				>
					{weekDate.map((day, idx) => (
						<div
							onClick={() => isDisable(viewDate, day) || day !== null && onclick(day)}
							className={[
								...
								day === null
									...
									: isDisable(viewDate, day)
										? 'text-Grey_Lighten-2 cursor-not-allowed'
										: isTodayAtCalendar(viewDate) === day
											? 'before:rounded-full before:border before:border-Grey_Lighten-3 cursor-pointer'
											: isSameDate(currentDate, viewDate) &&
												  formatDateToObject(currentDate).day &&
												  +formatDateToObject(currentDate).day === day
												? 'font-bold text-white before:rounded-full before:border-none before:bg-Blue_C_Default cursor-pointer'
												: 'hover:before:rounded-none hover:before:border-none hover:before:bg-Blue_C_Lighten-5 cursor-pointer',
							].join(' ')}
							key={`day-${day}-${idx}`}
						>
							{day}
